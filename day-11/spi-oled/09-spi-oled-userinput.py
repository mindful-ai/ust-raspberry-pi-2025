...
def main():
    ...
    try:
        while True:
            ...
            draw.rectangle((0, 0, width, height), fill=0)  # Clear background

            print("Enter text lines to display (max 8 lines). Empty line to display and input again, Ctrl+C to quit:")
            lines = []
            while len(lines) < 8:  # Max 8 lines (8 px font height * 8 lines = 64 px height)
            ...
            for i, text_line in enumerate(lines):
                draw.text((0, i * 8), text_line, font=font, fill=255)
